...
public class HandPositionPublisher : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timeElapsed > publishMsgFreq)
        {
            ...
            Vector3 tmp_rot = wholeBodyGround2handle_angleAxis(ground2handle_right_hand_rotm, ground2HMD_right_hand_rotm, rotation);
            // 位置向量(HMD到手把，再旋轉到機器人基底座標)
            Vector3 tmp_posVec_HMD2handle = tmp_handle_pos - tmp_HMD_pos;
            Vector3 tmp_pos = wholeBodyGround2handle_pos(tmp_posVec_HMD2handle, ground2HMD_rotm);
    ...
    private Vector3 wholeBodyGround2handle_angleAxis(Matrix3x3 ground2handle_right_hand_rotm, Matrix3x3 ground2HMD_right_hand_rotm, UnityEngine.Quaternion rotation)
    {
        ...
        Matrix3x3 wholeBodyGround2handle_rotm = robotBase2handle_rotm * wholeBodyGround2robotBase_rotm;
        float trace = wholeBodyGround2handle_rotm[1, 1] + wholeBodyGround2handle_rotm[2, 2] + wholeBodyGround2handle_rotm[3, 3];
        float angle = (float)(Math.Acos((trace - 1) / 2));
        Vector3 axis = new Vector3();
        axis[0] = (float)((1 / (2 * Math.Sin(angle))) * (wholeBodyGround2handle_rotm[3, 2] - wholeBodyGround2handle_rotm[2, 3]));
        axis[1] = (float)((1 / (2 * Math.Sin(angle))) * (wholeBodyGround2handle_rotm[1, 3] - wholeBodyGround2handle_rotm[3, 1]));
        axis[2] = (float)((1 / (2 * Math.Sin(angle))) * (wholeBodyGround2handle_rotm[2, 1] - wholeBodyGround2handle_rotm[1, 2]));
        return angle * axis;
    }
